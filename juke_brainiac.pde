...
//  set pins to connect with mechanism control unit
/*
        mechanism control pinout to brain, 1-17
        ardpin, mcupin - description
            1 - cancel signal, tied to mute trigger
            2 - on signal
            3 - (null) or +28vdc hot?
            4 - logic common
            5 - inner cam switch
            6 - outer cam switch
            7 - money counter signal
            8 - play counter signal
            9 - toggle signal 
            10 - optical switch index
            11 - optical switch home
            12 - +8vdc
            13 - turntable motor signal
            14 - transfer motor signal
            15 - mag motor signal
            16 - detent signal
            17 - "key"
...
// 2. The CCC sends clock and data signals to the digital display (nixie?), causing the
//	LED's to light.
//  set pins for outputs
//  3 digit nixie display requires minimum 7 pins DOut
...
// ----------
//A customer inserts a tokencoin , standard credit established, and credit is set
//	at 1 play for 10c, 6 plays for 50c and 13 plays for $1.
// 1. After customer inserts a coin into slot, the coin passes through the
//	validator and actuates the 10c switch.
// 2. The CCC senses the switch closure and stores 5 money units (nickels) in 
//	its memory.
// 3. Two pulses are sent to the money counter. clunk a solenoid
...
// increment playcount of track in eeprom
